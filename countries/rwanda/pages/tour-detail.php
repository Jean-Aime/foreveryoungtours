...
// Get related tours (same country only, excluding current tour)
$related_stmt = $pdo->prepare("
    SELECT t.*, c.name as country_name 
    FROM tours t 
    LEFT JOIN countries c ON t.country_id = c.id 
    WHERE t.status = 'active' AND t.id != ? AND t.country_id = ? 
...
<div class="min-h-screen bg-white pt-20">
    ...
    // Collect all images in priority order
    if (!empty($tour['cover_image'])) $gallery_images[] = $tour['cover_image'];
    if (!empty($tour['image_url'])) $gallery_images[] = $tour['image_url'];
    
    // Parse JSON fields - images array
    if (!empty($tour['images'])) {
        $images_data = json_decode($tour['images'], true);
        if (is_array($images_data)) {
            foreach ($images_data as $img) {
                if (!empty($img)) $gallery_images[] = $img;
            }
        }
    }
    
    // Parse JSON fields - gallery array
    ...
    <section class="max-w-7xl mx-auto px-4 py-8">
        <div class="nextcloud-card p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <?php 
                // Fix relative paths for display
                $image_src = $image;
                if (strpos($image, 'uploads/') === 0) {
                    $image_src = '../../../' . $image;
                } elseif (strpos($image, 'http') !== 0 && strpos($image, '/') !== 0) {
                    $image_src = '../../../uploads/tours/' . $image;
                }
                ?>
                <div class="relative overflow-hidden rounded-lg cursor-pointer hover:shadow-lg transition-shadow" onclick="openImageModal(<?php echo $index; ?>)">
...
function updateModalImage() {
    ...
    if (imageSrc.startsWith('uploads/')) {
        ...
        imageSrc = '../../../uploads/tours/' + imageSrc;
    }
