...
$country_code = str_replace('visit-', '', $folder_name);
$stmt = $pdo->prepare("SELECT c.*, r.name as region_name FROM countries c LEFT JOIN regions r ON c.region_id = r.id WHERE c.country_code = ? AND c.status = 'active'");
$stmt->execute([strtoupper($country_code)]);
$country = $stmt->fetch();
